...
namespace Genius.Controls.TreeView.Core
{
	...
	internal class Node : INode, INodeEnumerable, IEnumerable
	{
		...
		NodeState INode.State
		{
			...
			set
			{
				...
				if (this.State != value)
				{
					...
					int check = (Set.Contains(value, NodeState.HasCheck) ? 1 : 0) -
						(Set.Contains(old, NodeState.HasCheck) ? 1 : 0);
					int aChecked = (Set.Contains(value, NodeState.Checked) ? 1 : 0) -
						(Set.Contains(old, NodeState.Checked) ? 1 : 0);
					int signaled = (Set.Contains(value, NodeState.Signaled) ? 1 : 0) -
					...
					if (oldVisible && IsVisible)
					{
						...
						if (aChecked != 0)
							UpdateTotalCheckedCount(this.Parent, aChecked);
						if (signaled != 0)
		...
		internal static void UpdateTotalCheckedCount(Node aNode, int nb)
		{
			if (nb == 0)
				return;
			while (aNode != null)
