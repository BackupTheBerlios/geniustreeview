 ...
 * + ajouter une interface de gestion pour le hint IHintWindow
 * + FixedRows
 *  
 * Version 2.0
 * + Architecture compatible Avalon
 * + séparation des noeuds et du visuel encore plus forte
 * *****************************/
namespace Genius.Controls
{
	...
	/// </summary>
	public delegate void OnNodeClickDelegate(GeniusTreeView Sender, NodeCellEventArgs e);
	...
	[ToolboxBitmap(typeof(GeniusTreeView), "Resources.treeview.bmp")]
	public class GeniusTreeView : System.Windows.Forms.Control
	{
		private const int FCheck_Size = 16;
		...
		/// </summary>
		[Category("Behavior")]
		[Description("permete d'afficher un hint sous la souris")]
		...
		/// </summary>
		[Description("utiliser cet évènement pour effctuer le tri sur des valeur, plutôt que sur le texte")]
		[Category("Behavior")]
		...
		public event OnNodeClickDelegate		OnCellMouseDown;
		...
		private void LoadCheckImage()
		{
			...
			LoadImages(FImageCheckList, checks, this.GetType().Assembly);
		}
		...
		private void LoadTreeImage()
		{
			...
			LoadImages(FImageTreeList, images, this.GetType().Assembly);
		}
		
		#region gestion des ensemble
		...
		private Node InternalBottomNode()
		{
			...
			{
				int h;
				FBottomNode = CalcBottomOfHeight(InternalTopNode(), this.TreeRectangle().Height, out h, true);
			}
			return FBottomNode;
		}
		...
		{
			int h;
			return CalcBottomOfHeight(InternalTopNode(), this.TreeRectangle().Height, out h, true);
		}
		...
		private Node InternalTopNode()
		{
			...
			{
				Node n = FRoot;
				if (FFixedRowsCount > 0)
				{
					for (int i=0; n != null && i < FFixedRowsCount; i++)
						n = NextVisibleNode(n);
				}
				if (n != null)
					FTopNode = NextVisibleNode(n);
			}
			*/
			if (FTopNode == null)
		...
		private Node GetNodeAt(Point aPt, out Point aNodePoint)
		{
			...
			//startY = FixedRowsHeight;
			startY = 0;
			aNodePoint = new Point(0,0);

			if (false /*aPt.Y < startY*/)
			{
				Result = NextVisibleNode(FRoot);
				while (Result != null)
				{
					if (aPt.Y < Result.Height)
						return Result;
					aPt.Y -= Result.Height;
		...
		internal void InvalidateNode(Node aNode)
		{
			...
			if (rank.CompareTo(rankTop) >= 0 && rank.CompareTo(rankBottom) <= 0)
			{
				...
				Rectangle rRect = this.ClientRectangle;
		...
		private void DrawLines(Node aNode, Graphics g, Point NodePoint, DrawingOption option)
		{
			...
			if (!Contains(option, DrawingOption.HideTreeLines))
			{
				...
				//ligne horizontale qui relie le noeud à sa ligne de vie
		...
		private void DrawCheck(Node aNode, Graphics g, Point NodePoint)
		{
			if (Contains(aNode.State, NodeState.HasCheck))
			{
				Image check;
				...
				if (aNode.TotalCheck != aNode.TotalChecked && aNode.TotalChecked > 0)
					check = FImageCheckList.Images[3];
					...
					check = FImageCheckList.Images[1];
				else
					check = FImageCheckList.Images[0];
		...
		private Node PaintTree(Graphics g, Rectangle aRect)
		{
			...
			if (h > 0)
			{
				if (aRect.Top < h)
				{
					int diff = h - aRect.Top;
					aRect.Y = h;
					aRect.Height -= diff;
				}
			}
			Node n = GetNodeAt(new Point(aRect.Left, aRect.Top), out NodePointOrg);
			return PaintTree(g, aRect, n, false, h);				
		}

		private Node PaintTree(Graphics g, Rectangle aRect, Node aNode, bool dragImage, int aOffsetY)
		{
			Rectangle rCell;
			...
			if (paintinfo.FNode != null && !dragImage)
			{
				...
				R.Width = this.ClientRectangle.Width + this.OffsetX - 0;
				...
				if (!DoBeforeNodePainting(paintinfo))
				{
					if (UseColumns)
					{
						if (Contains(paintinfo.DrawingOptions, DrawingOption.ShowHorzLines))
						{
							...
							{
								int maxwidth = Math.Min(FHeader.Width - FScrolls.OffsetX, this.ClientRectangle.Width);
						...
						try
						{
							...
							for (int i=0; i < displays.Length; i++)
							{
								...
								if (fixedWidth > 0 && i >= FHeader.FixedColumnCount)
									g.ExcludeClip(new Rectangle(0,0, fixedWidth, this.ClientRectangle.Height));
			...
			g.DrawLine(Pens.Green, FScrolls.RangeX - FScrolls.OffsetX, 0, FScrolls.RangeX - FScrolls.OffsetX, this.ClientRectangle.Bottom);
#endif
			return bottom;
		}
		...
		{
			if (!paintinfo.BackColor.IsEmpty && !(FullRowSelect && paintinfo.FNode.Selected))
			{
			...
			if ((paintinfo.DisplayColumn == this.MainColumnIndex && UseColumns) || paintinfo.DisplayColumn == Constantes.NoColumn)
			{
				...
				DrawCheck(aNode, paintinfo.graphics, pt);
				if (Contains(aNode.State, NodeState.HasCheck))
					pt.X += FCheck_Size;
		...
		private void DoPaintCellText(PaintInfo paintinfo)
		{
			...
			bool isSelected = Contains(paintinfo.FNode.State, NodeState.Selected) && paintinfo.DisplayColumn == FNodes.CurrentColIndex;
			if (!FullRowSelect && isSelected)
			{
				if (!Contains(paintinfo.DrawingOptions, DrawingOption.AlwaysHideSelection))
				{
					if (this.Focused || !Contains(paintinfo.DrawingOptions, DrawingOption.HideSelection))
					{
						DrawSelectedRectangle(paintinfo.graphics, paintinfo.FNodeRect, false);
					}
					else
						isSelected = false;
				}
			...
			{
				if (isSelected ||(paintinfo.FNode.Selected && FullRowSelect))
					aTextColor = FColors.SelectedTextColor.GetBrush();
				else
					aTextColor = FColors.TextColor.GetBrush();
			}
			else
				aTextColor = paintinfo.ForeColor.GetBrush(paintinfo.FNodeRect);

			StringFormat sf = paintinfo.StringFormat;
			if (sf == null)
				sf = GetStringFormat(paintinfo.FNode, paintinfo.DisplayColumn);
			Rectangle rText = paintinfo.FNodeRect;
			//TODO: demander la font pour le noeuds
			Font f = paintinfo.Font;
			if (f == null)
				f = this.Font;
			paintinfo.graphics.DrawString(sText, f, aTextColor, rText, sf);
			if (!FullRowSelect && isSelected && this.Focused && !Contains(paintinfo.DrawingOptions, DrawingOption.HideFocusRect))
				DrawFocusedRectangle(paintinfo.graphics, paintinfo.FNodeRect);
		}
		...
		private void DoAfterNodePainting(PaintInfo paintinfo)
		{
			...
			{
				if (FullRowSelect && paintinfo.FNode.Selected)
					DrawFocusedRectangle(paintinfo.graphics, paintinfo.FNodeRect);
			}
		}
		...
		private void DoPaintNodeBackGround(PaintInfo paintinfo)
		{
			...
			if (paintinfo.DefaultDrawing)
			{
				if (FullRowSelect && paintinfo.FNode.Selected && !Contains(paintinfo.DrawingOptions, DrawingOption.AlwaysHideSelection))
					DrawSelectedRectangle(paintinfo.graphics, paintinfo.FNodeRect, false);
			}
		}
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			try
			{
				Rectangle aRect;
				Node tmp;
				aRect = e.ClipRectangle;
				Debug.WriteLine(string.Format("Rectangle d'invalidation : {0}", aRect));
				if (aRect.IsEmpty)
					aRect = this.ClientRectangle;
				tmp = PaintTree(e.Graphics, aRect/*, options */);
				if (aRect.Equals(this.ClientRectangle))
					FBottomNode = tmp;
			}
			catch(Exception ex)
			{
		...
		private bool ShowSelectedColumn()
		{
			...
			left -= FScrolls.OffsetX ;
			right -= FScrolls.OffsetX;
			...
			else if (left > 0 && right > ClientRectangle.Right)
			{
				left = Math.Min(left, right - ClientRectangle.Right);
		...
		private void GotoColumn(int aColIndex)
		{
			...
			if (InternalSelectNode(selected, aColIndex))
			{
		...
		*/
		...
		private bool InternalSelectNode(Node n, int colIndex)
		{
			...
			if (n != oldSelected || (UseColumns && colIndex != FNodes.CurrentColIndex && !FFullRowSelect))
			{
				...
				canSelect = FNodes.InternalSelectNode(n, false, colIndex);
			}
			else 
				canSelect = false;
			return canSelect;
		...
		{
			bool Result = InternalSelectNode(n, aCol);
			if (Result)
			{
...
*/
		...
		private void ScrollUp(int h)
		{
			if (FTopNode != null && FScrolls.CanScrollUp())
			{
				int realHeight;
				/*
				Debug.WriteLine("Begin ScollUp : "+ FScrolls.OffsetY.ToString());
				if (FTopNode != null)
					Debug.WriteLine("top : " + FTopNode.TopPosition.ToString());
				*/
				FTopNode = CalcTopOfHeight(FTopNode, h, out realHeight, true);
				/*
				if (FTopNode != null)
					Debug.WriteLine("top(afterscroll) : " + FTopNode.TopPosition.ToString());
				*/
		...
		private void ScrollDown(int h)
		{
			if (FTopNode != null && FScrolls.CanScrollDown())
			{
				int realHeight = 0;
				/*
				Debug.WriteLine("Begin ScollDown : " + FScrolls.OffsetY.ToString());

				if (FTopNode != null)
					Debug.WriteLine("top : " + FTopNode.TopPosition.ToString());
				*/
				Node bottom = InternalBottomNode();
				bottom = CalcBottomOfHeight(bottom, h, out realHeight, true);
				FTopNode = CalcBottomOfHeight(FTopNode, realHeight, out realHeight, false);
				if (realHeight == 0 && bottom != null && NextVisibleNode(bottom) != null)
				{
					FTopNode = NextVisibleNode(FTopNode);
					realHeight = FTopNode.Height;
				}
				/*
				if (FTopNode != null)
					Debug.WriteLine("top(afterscroll) : " + FTopNode.TopPosition.ToString());
				*/
				FScrolls.OffsetY += realHeight; 
				//Debug.WriteLine("End ScollDown : " + FScrolls.OffsetY.ToString());
				UpdateHorizontalScrollBar();
				InvalidateTree();
			}
		}

		private bool ScrollDownBy(int nb)
		{
			if (FTopNode != null && FScrolls.CanScrollDown())
			{
				ScrollDown(FTopNode.Height);
				return true;
			}
			return false;
		}
		...
		private IHintWindow CreateHintWindow()
		{
			if (OnCreateHintWindow != null)
			{
				NodeHintWindowEventArgs e = new NodeHintWindowEventArgs();
				OnCreateHintWindow(this, e);
				return e.HintWindow;
			}
			return new BaseHintWindow(this);
		}
		...
		{
			Node n = drgevent.Data.GetData(typeof(Node)) as Node;
			if (n != null && FDropNode != null)
			{
				FDropHere = true;
				this.BeginUpdate();
				try
		...
		private void PrepareDrag(Point aStart, HitNodeInfo ahit)
		{
			...
			using (Graphics g = Graphics.FromImage(bmp))
			{
				...
				try
				{
					...
					PaintTree(g, rNode, ahit.HitNode, true, 0);
				}
				finally
				{
		...
		private void FillTree()
		{
			...
			if (FDataSource != null && AutoSort)
				SortTree(FHeader.SortColumn, FHeader.SortingDirection);
		}
		#endregion
		...
		private StringFormat GetStringFormat(Node aNode, int aColumn)
		{
			...
			if (UseColumns)
				sf.Alignment = FHeader.DisplayColonnes(aColumn).TextAlignment;
			return sf;
		}
		...
		private void DetermineHitPositionLTR(ref HitNodeInfo aHi, int offset, int right, StringAlignment alignment)
		{
			...
			else
			{
				...
				{
					imageoffset = indent;
					if (FImageCheckList != null && Contains(aHi.HitNode.State, NodeState.HasCheck))
					{
		...
		private void GetHitTestInfoAt(int X, int Y, bool Relative, ref HitNodeInfo aHi)
		{
			...
			else if ( (X + FScrolls.OffsetX > FScrolls.RangeX) || (X > this.ClientSize.Width))
				Include(ref aHi.HitPositions, HitPositionsEnum.ToRight);
			if (Y < 0)
				Include(ref aHi.HitPositions, HitPositionsEnum.Above);
			else if ( (Y + FScrolls.OffsetY > FScrolls.RangeY) || (Y > this.ClientSize.Height))
				Include(ref aHi.HitPositions, HitPositionsEnum.Below);

			if (aHi.HitPositions == (HitPositionsEnum)0)
			{
				Point aNodePoint;
				...
				{
					ColRight = Math.Max(FScrolls.RangeX, ClientSize.Width);
					aHi.HitColumn = Constantes.NoColumn;
				}
				ColLeft = 0;
				...
				else
				{
					...
					{
						currenAlignment = FHeader.DisplayColonnes(aHi.HitColumn).TextAlignment;
					}
		...
		private void HandleMouseDown(MouseEventArgs e, HitNodeInfo aHi)
		{
			...
			*/
			...
			if (!InEdit || DoEndEdit())
			{
				...
				if (Contains(aHi.HitPositions, HitPositionsEnum.OnItemLabel) ||
					UseColumns)
				{
					newColumn = FNodes.CurrentColIndex != aHi.HitColumn;
					column = aHi.HitColumn;
				}
				else
				{
					newColumn = false;
					column = FNodes.CurrentColIndex;
				}
		...
		protected override void OnMouseDown(MouseEventArgs e)
		{
			...
			GetHitTestInfoAt(e.X, e.Y, true, ref hitInfo);
			if (e.Clicks >= 2)
			{
			...
			if (OnCellMouseDown != null)
				OnCellMouseDown(this, new NodeCellEventArgs(hitInfo.HitNode, hitInfo.HitColumn));
		}
		...
		protected override void OnMouseMove(MouseEventArgs e)
		{
				...
				&& AllowDrag)
			{
				...
				DragDropEffects effects = DoDragDrop(hitInfo.HitNode.Clone(true), DragDropEffects.All);
		...
		private bool GetHintText(HitNodeInfo hitInfo, ref string aText)
		{
			...
			if (aText == null || aText.Length == 0)
			{
				if (!UseColumns || ( UseColumns && hitInfo.HitColumn == MainColumnIndex))
				{
					...
					bool showhint = false;
					//Debug.WriteLine(String.Format("rText.right : {0} : {1} : {2}", rText.Right, this.ClientRectangle.Width, this.ClientSize.Width));
					showhint = rText.Right > this.ClientSize.Width;
					if (showhint)
			...
			switch((VirtualKeys)e.KeyValue)
			{
				...
				case VirtualKeys.VK_PRIOR :
					if (Selected != null)
					{
						...
						{
							int h = 0;
							CalcTopOfHeight(InternalTopNode(), TreeRectangle().Height, out h, true);

							Node newSelected = CalcTopOfHeight(Selected, h, out h, true);
				...
				case VirtualKeys.VK_UP :
					if (Selected != null)
					{
						...
						do
						{
							...
							if (tmp != null && InternalSelectNode(tmp, FNodes.CurrentColIndex))
							{
								if (!ShowNode(tmp))
								{
		...
		{
			//TODO: vérifié que selected est visible, ou le rendre visible en X, et en Y
			if (Selected != null && !InEdit && CanEdit(Selected))
			{
				FTimers.Stop(KindTimer.Edit);
				HideHint();
				...
				Rectangle rNode = GetTextRect(Selected, FNodes.CurrentColIndex);
				if (!UseColumns)
					rNode.Width += 20;
				/*
				if (rNode.Bottom > this.ClientRectangle.Bottom)
					if (ScrollDownBy(1))
						rNode.Offset(0, -FTopNode.Height);
				*/
				DoInitEdit(FEditor, Selected, FNodes.CurrentColIndex);
				FEditor.OnKeyDown +=new KeyEventHandler(Editor_OnKeyDown);
				FEditor.BeginEdit(Selected, FNodes.CurrentColIndex, rNode);
				return true;
			}
			return false;
		}

		private void DoInitEdit(ITreeViewEdit editor, INode n, int aDisplayColumn)
		{
			if (OnInitEdit != null)
				OnInitEdit(this, new EditEventArgs(n, aDisplayColumn, editor));
		}
		...
		private string GetNodeText(Node aNode, int aColIndex)
		{
			if (FDataIsStringProvider)
			{
				...
				if (stringProvider != null)
					return stringProvider.GetText(UseColumns ? aColIndex : -1);
				else
					return string.Empty;
			}
			else
		...
		private bool CanEdit(Node aNode)
		{

			if (OnBeforeEdit != null)
			{
				CanEditEventArgs ev = new CanEditEventArgs(aNode);
...
Value of the high-order word of wParam. Specifies the current position of the scroll box if the nScrollCode parameter is SB_THUMBPOSITION or SB_THUMBTRACK; otherwise, nPos is not used. 
				...
				switch(scrollcode)
				{
					...
					case 1 : //SB_LINEDOWN
						...
						break;
					case 2 : //SB_PAGEUP
						ScrollUp(this.ClientRectangle.Height);
						break;
					case 3 :
						ScrollDown(this.ClientRectangle.Height);
						break;
			...
			else if (m.Msg == 276) //WM_HSCROLL
			{
				...
				switch(scrollcode)
				{
					case 0 :
						...
						break;
					case 1 :
						maxx = FScrolls.RangeX - this.ClientRectangle.Width;
					...
					case 2 :
						if (FScrolls.OffsetX > 0)
						{
							FScrolls.OffsetX -= this.ClientRectangle.Width;
							if (FScrolls.OffsetX < 0)
								FScrolls.OffsetX = 0;
							InvalidateTree();
						}
		...
		public INode Add(INode aParent, string aText, object aData)
		{
			...
			if (FUpdateCount == 0)
			{
				if (AutoSort)
				{
					...
					try
					{
						FNodes.DoSort((Node)aParent, FHeader.SortColumn, FHeader.SortingDirection, new OnCompareNodeDelegate(this.DoCompare), false);
					}
					finally
					{
						FCachedText = null;
		...
		public void SetVisibleNode(INode aNode, bool value)
		{
			CancelEdit();
			Node n = (Node)aNode;
			if (Set.Contains(n.State, NodeState.Selected) && value == false)
				FLastClick = new Point(-1,-1);
			int aHeight;
		...
		{
			HitNodeInfo hitInfo = new HitNodeInfo();
			GetHitTestInfoAt(pt.X, pt.Y, false, ref hitInfo);
		...
		private IComparable DoGetNodeValue(INode A, int acol)
		{
			IComparable aValue;
			if (FCachedText != null && FCachedText.Contains(A))
				aValue = (IComparable)FCachedText[A];
			else
			{
				aValue = OnGetNodeValue(A, acol);
				if (FCachedText != null)
					FCachedText[A] = aValue;
			}
			return aValue;
		}
		...
		{
			if (aDisplayCol > Constantes.NoColumn)
			{
				FHeader.SetSortColumn(Header.DisplayIndexToIndex(aDisplayCol), aDirection);
				SortChildNodes(FRoot, aDisplayCol, aDirection, true);
			}
		}

		
		public void SortChildNodes(INode aNode, int aCol, SortDirection aDirection, bool recurse)
		{
			FUpdateCount++;
			try
			{
				if (aCol > Constantes.NoColumn)
				{
					...
					try
					{
						FNodes.DoSort((Node)aNode, aCol, aDirection, new OnCompareNodeDelegate(this.DoCompare), recurse);
						RecalculTopNode();
						FBottomNode = null;
					}
					finally
		...
		public void DrawSelectedRectangle(Graphics g, Rectangle aRect, bool drawFocused)
		{
			GeniusLinearGradientBrush geniusBr;

			geniusBr = this.Focused ? FColors.SelectedColor : FColors.SelectedUnfocusedColor;
			using (Brush br = geniusBr.GetBrush(aRect))
			{
				g.FillRectangle(br, aRect);
			}
			if (drawFocused)
				DrawFocusedRectangle(g, aRect);
		}
		...
		/// <param name="g"></param>
		/// <param name="aRect"></param>
		public void DrawFocusedRectangle(Graphics g, Rectangle aRect)
		{
			Rectangle r = aRect;
			r.Width -= 1;
			r.Height -=1;
			g.DrawRectangle(this.Focused ? Colors.FocusedRectangleColor : Colors.UnFocusedRectangleColor, r);
		}
		#endregion
		#endregion
